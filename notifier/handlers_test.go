	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, tPoint := range test.notificationTimes {
				url := fmt.Sprintf("/notify?affectedGraphId=%s&modifiedGraphId=%s&lastChangeDate=%s%s", ftStagingModel, ftStagingModel, today, tPoint)
...
func TestGettingSmartlogicChangesOneRequestAtATime(t *testing.T) {
	...
	today := time.Now().Format(TimeFormat)
	requestURI := fmt.Sprintf("/notify?affectedGraphId=%s&modifiedGraphId=%s&lastChangeDate=%s", ftStagingModel, ftStagingModel, today)
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			kc := &mockKafkaClient{}
			sl := &mockSmartlogicClient{
